...
const FULFILLMENT_API_URL = process.env.FULFILLMENT_API_URL || "http://localhost:8080";
...
server.tool("getInventory", "Get product inventory", async () => {
  ...
  const res = await fetch(`${FULFILLMENT_API_URL}/inventory`);
...
server.tool(
  ...
  async ({ items, customerName }) => {
    ...
    const res = await fetch(`${FULFILLMENT_API_URL}/purchase`, {
      method: "POST",
      headers: {
